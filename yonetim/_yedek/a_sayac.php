	...
	<div class="content-wrapper">
		...
		<section class="content">
			<div class="container-fluid">
				<div class="row">
					<?php
					if ($sonuc = $mysqli -> query("select s.*, d.adi as dil_adi from diller d left join sayac s on d.DilKodu = s.dil")) {
								...
								<div class="form-group">
									<select name="dil" class="form-control m-bot15">
										...
										if ($sonuc1 = $mysqli -> query("select id, adi, DilKodu from diller")) {
											...
											?>
										<option value="<?=$row1['DilKodu']?>" <?if ($row['dil']==$row1['DilKodu']){echo "selected";}?>><?=$row1['adi']?></option>
											<?
											}
										}
										?>
