  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0"><?php if ($k==2){echo "Blog Yönetim";}  elseif($k==100) {echo "Vİdeo Yönetim";} elseif($k==205) {echo "SSS Yönetim";} elseif($k==209) {echo "Hizmetlerimiz Yönetim";} elseif($k==20) {echo "Yazar Yönetim";}  elseif($k==2000) {echo "Otel Oda Yönetim";}   else {echo "Üst Menüler Yönetim"; } ?></h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="index.php">Anasayfa</a></li>
              <li class="breadcrumb-item active"><?php if ($k==2){echo "Blog Yazıları";}  elseif($k==100) {echo "Video Listesi";} elseif($k==205) {echo "SSS Yönetim";} elseif($k==209) {echo "Hizmetlerimiz Yönetim";}  elseif($k==2000) {echo "Otel Oda Yönetim";} else {echo "Üst Menüler"; } ?></li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
...
if (isset($_POST['sayfakaydet']) ) {
            ...
            $sayfaadi=mysqli_real_escape_string($mysqli, $_POST['sayfaadi']);


            if(isset($_POST['sayfapurl']))   {  $sayfapurl=$_POST['sayfapurl'];  } else {$sayfapurl=NULL; }
            if(isset($_POST['sayfatitle']))   {  $sayfatitle=mysqli_real_escape_string($mysqli, $_POST['sayfatitle']);  } else {$sayfatitle=NULL; }
            ...
            if(isset($_POST['sayfadescr']))   {  $sayfadescr=mysqli_real_escape_string($mysqli, $_POST['sayfadescr']);  } else {$sayfadescr=NULL; }
            if(isset($_POST['sayfaicerik']))   {  $sayfaicerik=mysqli_real_escape_string($mysqli, $_POST['sayfaicerik']);  } else {$sayfaicerik=NULL; }
            ...
            if(isset($_POST['sayfakategori']))   {  $sayfakategori=$_POST['sayfakategori'];  } else {$sayfakategori="0"; } 
           ...
           //  EDITLEME ISE
            ...
            // ESKİYİ EDİTLEME BAŞ
                  $sql = "UPDATE sayfalar  SET
                   ...
                   sira = '$sayfasira', icerik = '$sayfaicerik', aktif = '$sayfaaktif', tur = '$tur', dil = '$sayfadili', sayfa_turu = '$sayfaturu',  kategori = '$sayfakategori',  konum = '$k', 
            ...
            if($action=="yenisayfa") {
                ...
                $sql = "INSERT INTO
                  ...
                  eklenme_tarihi, yazar, kodfoot, kodhead, cano, indexle, icon,kategori ) VALUES ('$sayfaustmenu', '$sayfaadi', '$sayfaurl',  '$sayfapurl', '$sayfatitle', 
                  '$sayfadescr', '$sayfakeywords',
                   ...
                   '$sayfakodhead', '$sayfacano', '$sayfaindexle','$sayfaicon','$sayfakategori')";
                if ($mysqli->query($sql) === TRUE) {
...
// ÖNCE SAYFA BİLGİLERİ BİR AL YİĞENİM ///
  ...
  $stmt_sayfa = $mysqli->prepare("SELECT uid, adi, url, purl,  title, descr, keywords,sira, res, icerik, aktif, tur, dil, sayfa_turu, kategori, urun_turu, marka, konum, vitrin, eklenme_tarihi, yazar, kodfoot, kodhead, cano, indexle, icon FROM sayfalar WHERE id = ? LIMIT 1") ;
  ...
  $stmt_sayfa->bind_result($ssayfauid, $ssayfaadi, $ssayfaurl, $ssayfapurl, $ssayfatitle, 
    ...
    $ssayfaaktif, $ssayfatur, $ssayfadil, $ssayfaturu,$ssayfakategorisi,$ssayfaurunturu,
