        ...
        <form id="sayfaform" action="<?php echo $formtur; ?>" method="POST">
          ...
          <?php } ?>
            ...
            <div class="form-group">
                    ...
                    if ($result_sayfaturcek = $mysqli -> query($sql_sayfaturcek)) {
                       while ($row_sayfaturcek= $result_sayfaturcek -> fetch_row()) {
                         ...
                         $sayfaturselected='';
                             ...
                             if($k =='22222'  &&  $sayfaturcek_id == 'blog') {
				...
				<div class="form-group">
					<label for="sayfakategori">Kategorisi</label>
					<select name="sayfakategori" id="aktif" class="form-control">
						<option value="0">- Kategorisi Yok -</option>
						<option disabled>──────────</option>
						<?php
						$sql_menu_cek="SELECT id, adi, dil, uid  FROM sayfalar WHERE sayfa_turu = 'Kategori' ORDER BY sira ASC";             
						if ($result_menu_cek = $mysqli -> query($sql_menu_cek)) {
							...
							?>
						<option <?php echo ( $ssayfakategorisi == $menucek_id ? " selected " : ""); ?> value="<?php echo $menucek_id; ?>"><?php echo $menucek_adi; ?></option>
						<?php
						}  // dış while
						} // dış if
						?>
          ...
          <div class="form-group">
              ...
              <div class="form-group">
               ...
               <label for="sayfaicon">İcon</label>
                <input langField="1" name="sayfaicon" id="sayfaicon" class="form-control" type="text" value="<?php if(isset($sayfaicon))
                {echo $sayfaicon;} ?>"  />
              </div>
...
$('#sayfatitle').keyup(function() {
  ...
  $('#count_message2').html(text_length2 + ' / ' + text_max2);
         ...
         maxlength="1000" placeholder="Tanımlama" rows="5" noresize><?php if(isset($ssayfadescr))
